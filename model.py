...
print("*****************Reading csv file " + def_loc_csvfile + "************************")
with open(def_loc_csvfile) as csvfile:
    ...
    for line in lineReader:
        ...
        csvData.append((def_loc_img+sCenterFileName,stearCenter,'None'))
        csvData.append((def_loc_img+sLeftFileName,stearCenter+correction_par,'None'))
        csvData.append((def_loc_img+sRightFileName,stearCenter-correction_par,'None'))
        ...
        csvData.append((def_loc_img+sCenterFileName,stearCenter,'Brightness'))
        ...
        # image array , additonal brightness change is added if required.
            for batch_sample in batch_samples:
                ...
                if batch_sample[2].lower()=="brightness":
                    ...
                    rdbrightness = .25 + np.random.uniform()
...
# add CNN layer with filter 5x5x24 followed by relu activation 
model.add(Convolution2D(24,5,5,subsample=(2,2),activation="relu"))
# add drop out to overcome over-fitting
model.add(Dropout(0.3))
# add CNN layer with filter 5x5x36 followed by relu activation
model.add(Convolution2D(36,5,5,subsample=(2,2),activation="elu"))
# add drop out to overcome over-fitting
model.add(Dropout(0.2))
# add CNN layer with filter 5x5x48 followed by relu activation
model.add(Convolution2D(48,5,5,subsample=(2,2),activation="relu"))
# add CNN layer with filter 3x3x64 followed by relu activation
...
# use MSE as loss function and optimizer as ADAM so that learing rate 
...
model.fit_generator(train_generator, samples_per_epoch=len(train_samples), validation_data=validation_generator,nb_val_samples=len(validation_samples),nb_epoch=30,verbose = 1,callbacks=[checkpoint,earlystopping])
